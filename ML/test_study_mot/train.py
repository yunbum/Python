...
def onehot_encode_label(path):
    onehot_label = unique_labels == get_label_from_path(path)
    onehot_label = onehot_label.astype(np.uint8)
    return onehot_label

## Training Data, image check
print(get_label_from_path(paths_training[0]))
image = np.array(Image.open(paths_training[0]))
print(image.shape)

## Test Data check
print(get_label_from_path(paths_test[0]))
image = np.array(Image.open(paths_test[0]))
print(image.shape)
...
print(unique_labels)
print(onehot_encode_label(paths_training[0]))
...
print(unique_labels)
print(onehot_encode_label(paths_test[0]))
...
#print('num_testfiles', num_testfiles)
TB_SUMMARY_DIR = './log'
...
with tf.Session() as sess:
    ...
    for i in range(STEPS):
        ...
        ## dt check
        end_t = time.clock()
        print('learning time =%3.3f \n' % (end_t - start_t))

    print('class n = ', num_testfiles// test_size)
